...
public class CalendarDateValidatorTests {
 ...
 //get end of week date as a string
        String returnDate = CalendarDateValidator.getEndOfWeek(stringCurrentDate);
//return date object
        Date endOfWeek = formatter.parse(returnDate);
...
//set calendar date to returned date
 ...
 //given any input, the output day of week should always be sunday
        assertEquals("Should return a Saturday",cal.get(Calendar.DAY_OF_WEEK), Calendar.SUNDAY);
...
//output must always be true if the input is the current date
        assertTrue("should be true since today is in the current week", CalendarDateValidator.isCurrentWeek(stringCurrentDate));
